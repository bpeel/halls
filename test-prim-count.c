...
init_vertices(struct data *data)
{
        ...
        for (int i = 0; i < N_TRIANGLES; i++) {
                float x = (i * 2 + 1.5f) * TRIANGLE_SIZE - 1.0f;

                p[0] = x - TRIANGLE_SIZE / 2.0f;
                p[1] = -TRIANGLE_SIZE / 2.0f;
                p[2] = x + TRIANGLE_SIZE / 2.0f;
                p[3] = -TRIANGLE_SIZE / 2.0f;
                p[4] = x;
                p[5] = TRIANGLE_SIZE / 2.0f;

                p += 6;
        }

        assert((uint8_t *) p == (uint8_t *) verts + sizeof verts);
