...
{
        struct drm_v3d_create_bo req = {
                .size = BUF_SIZE,
        };
        int ret;

        ret = drmIoctl(dev_fd, DRM_IOCTL_V3D_CREATE_BO, &req);
...
{
#if 0
        struct drm_v3d_label_bo req = {
                .handle = buf,
                .len = strlen(BUF_LABEL),
                .name = (uintptr_t) BUF_LABEL,
        };
        int ret = drmIoctl(dev_fd, DRM_IOCTL_V3D_LABEL_BO, &req);
        ...
        uint32_t buf)
{
        ...
        struct drm_v3d_mmap_bo req = {
                .handle = buf,
        };

        ret = drmIoctl(dev_fd, DRM_IOCTL_V3D_MMAP_BO, &req);
        if (ret == -1) {
                fprintf(stderr, "v3d_mmap_bo failed: %s\n", strerror(errno));
                return false;
        }
